...
UID_SUPPORT_GROUP_MESSAGES, TYPE_MSG_IMAGE, START_HIDDEN_MESSAGE, DURATA_SESSIONE } from '../utils/constants';
// utils
...
export class GenericMessagingService {
  ...
  // data di scadenza della sessione
  expiredSession: any;
  durataSessione: number = DURATA_SESSIONE;
  ...
  connect(conversationWith) {
    ...
    this.expiredSession = this.storageService.getItem('expiredSession');
    ...
    if (this.sessionUid) {
      ...
      if (this.expiredSession) {
        ...
        if (timestamp <= this.expiredSession) {
          //la sessione non è scaduta
          const now: Date = new Date();
          now.setMinutes(now.getMinutes() + this.durataSessione);
          const timestamp = now.valueOf();
          this.expiredSession = timestamp;
          this.storageService.setItem('expiredSession', this.expiredSession);
        } else {
          //la sessione è scaduta, ne creo un'altra
