...
public class AlpacaService {
	...
	public DichiarazioneRisposta sendData(Richiesta richiesta, Procedura procedura, UserPreferences userPreferences) throws PortalException {
		...
		if (organization != null && organization.isRoot()) {
			...
			Map<TipoIntegrazione, Map<String, it.servizidigitali.presentatoreforms.common.service.integration.output.IntegrationService>> outputBackofficeIntegrationServicesMap = integrationServiceFactory
					.getOutputBackofficeIntegrationServicesMap();

			Map<String, it.servizidigitali.presentatoreforms.common.service.integration.output.IntegrationService> integrationServicesMap = outputBackofficeIntegrationServicesMap
					.get(tipoIntegrazione);
			if (integrationServicesMap != null && !integrationServicesMap.isEmpty()) {
				it.servizidigitali.presentatoreforms.common.service.integration.output.IntegrationService integrationService = integrationServicesMap.get(servizio.getCodice());
