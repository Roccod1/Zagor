...
public class AllegatoRichiestaService {
			...
			if (Validator.isNotNull(allegato)) {
				...
				if (Validator.isNotNull(allegato)) {
					...
					String idDocumentale = fileServiceFactory.getActiveFileService().saveRequestFile(allegato.getName(), allegato.getName(), allegato.getName(), codiceServizio, stream, mimeType,
							userId, groupId);
					creaAllegatoRichiesta(idDocumentale, richiestaId, false, userName, groupId, userId);
				}
	...
	public void creaAllegatoRichiesta(String idDocumentale, long richiestaId, boolean principale, String userName, long groupId, long userId) {
		...
		try {
			if (Validator.isNotNull(allegato)) {
				...
				if (Validator.isNotNull(allegato)) {
					...
					if (Validator.isNotNull(idDocumentale)) {
						creaAllegatoRichiesta(idDocumentale, richiestaId, true, userName, groupId, userId);
					}
				}
			}
		}
