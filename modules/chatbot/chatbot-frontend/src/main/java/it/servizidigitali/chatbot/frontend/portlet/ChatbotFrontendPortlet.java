...
public class ChatbotFrontendPortlet extends MVCPortlet {
	...
	@Reference
	private ChatbotFrontendService chatbotFrontendService;
	...
	public void render(RenderRequest renderRequest, RenderResponse renderResponse) throws IOException, PortletException {
		...
		try {
			...
			ServizioEnte servizioEnteByPage = chatbotFrontendService.getServizioEnteByPage(themeDisplay);

			if (servizioEnteByPage != null) {
				chatbotEnable = chatbotFrontendService.isChatbotaAttivo(servizioEnteByPage);
			}

			// Verifica pagina servizio abilitata all'utilizzo del chatbot

			// MultiValueMap<String, String> parameters =
			// UriComponentsBuilder.fromUriString(currentUrl).build().getQueryParams();
			...
			// if (parameters.containsKey(PortletConstants.PARAMETRO_ID_SERVIZIO) ||
			...
			// }
			// }
			// if (comuneISAServizioSelected != null && comuneISAServizioSelected.getComuneISA()
			// != null && comuneISAServizioSelected.isAttivo()
			// && comuneISAServizioSelected.getComuneISA().getAttivo()) {
			// chatbotEnable = comuneISAServizioSelected.isChatbot();
			// if (chatbotEnable) {
			// String configurazioneChatbotJsonString =
			// comuneISAServizioSelected.getConfigurazioneChatbot();
			// if (configurazioneChatbotJsonString != null &&
			// !configurazioneChatbotJsonString.isEmpty()) {
			// ConfigurazioneChatbot configurazioneChatbot =
			// configurazioneChatbotConverter.convertToEntityAttribute(configurazioneChatbotJsonString);
			// if (configurazioneChatbot != null) {
			// // Aggiunta/sovrascrittura customAttributes
			// if (configurazioneChatbot.getCustomAttributes() != null &&
			// !configurazioneChatbot.getCustomAttributes().isEmpty()) {
			// customAttributes.putAll(configurazioneChatbot.getCustomAttributes());
			// if
			// (configurazioneChatbot.getCustomAttributes().containsKey(CustomAttribute.AGENT.getValue()))
			// {
			// chatbotConfigurationModel.setAgentId((String)
			// configurazioneChatbot.getCustomAttributes().get(CustomAttribute.AGENT.getValue()));
			// }
			// }
			//
...
//							.build();
			...
			String buttonLabel = LanguageUtil.get(themeDisplay.getLocale(), keyChatbotBtnLabel); // rb.getString("label.customAttributes."
																									// +
																									// CustomAttribute.BUTTON_LABEL.name())
			String recipientName = LanguageUtil.get(themeDisplay.getLocale(), keyChatbotRecipientName); // rb.getString("label.customAttributes."
																										// +
																										// CustomAttribute.RECIPIENT_FULLNAME.name())
			String welcomeSummary = LanguageUtil.get(themeDisplay.getLocale(), keyChatbotWelcomeSummary); // rb.getString("label.customAttributes."
																											// +
																											// CustomAttribute.WELCOME_SUMMARY.name())
			...
			customAttributes.put(CustomAttribute.BUTTON_LABEL.getValue(), buttonLabel);
			customAttributes.put(CustomAttribute.WELCOME_IMAGE.getValue(), welcomeImage);
			customAttributes.put(CustomAttribute.WELCOME_SUMMARY.getValue(), welcomeSummary);
